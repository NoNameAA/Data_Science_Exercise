...
def pivot_months_pandas(data):
	...
	# ...
	new_data = data.copy()
	new_data[r'date'] = new_data[r'date'].apply(date_to_month)
	grouped_data = new_data.groupby(['name', 'date'])['precipitation'].sum().reset_index()

	counts = new_data
	counts['counts'] = pd.Series(1, index=counts.index)
	grouped_counts = counts.groupby(['name', 'date'])['counts'].sum().reset_index()

	totals_data = grouped_data.pivot(index='name', columns='date', values='precipitation')
	counts_data = grouped_counts.pivot(index='name', columns='date', values='counts')

	return totals_data, counts_data
...
def main():
	...
	np.savez('monthdata_test.npz', totals=totals.values, counts=counts.values)
